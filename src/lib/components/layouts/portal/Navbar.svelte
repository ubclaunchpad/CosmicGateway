...
<div class="h-full pt-14 flex flex-col">
	...
	>
		...
		<ul
			class=" flex flex-col gap-4 justify-start items-center w-full transition-all h-full duration-300 px-4"
		>
			{#each links as link}
				<li
					class="p-2 w-full rounded flex justify-center items-center transition-all duration-200 hover:bg-gray-100"
				>
		...
		<div class="flex flex-row justify-between items-center w-full border-t border-gray-200 p-2">
			...
			<div class="relative inline-block text-left">
				...
				{#if dropdownOpen}
					<div
						class="absolute right-0 bottom-full mb-2 w-40 origin-bottom-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none"
						role="menu"
