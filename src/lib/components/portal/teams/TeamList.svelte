	...
	<div class=" justify-start items-center gap-4 flex flex-row overflow-scroll w-full">
		...
		{:else}
			{#each teams as team}
				<a
					transition:fade={{ duration: 300 }}
					class="card flex bg-base-100 border border-base-200 rounded-lg h-56 w-56 overflow-hidden flex-col flex-shrink-0 relative text-white"
					href={`/teams/${team.label}-${team.id}`}
				>

                  <img class="h-full w-full object-cover absolute " src={team.image_link? team.image_link : 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRoDPlpGIQ3dL4B72cLafz5O3dpT3O_WzwZ6STP-CHu4w&s'} alt={team.label}  />
					...
					>
						...
						<p class="text-neutral-200 overflow-scroll line-clampx-6 flex-1 text-sm">{team.description}</p>
					</div>
				</a>
			{/each}
		{/if}
