<script lang="ts">
	...
	const roles = ['developer', 'designer', 'tech lead', 'other'];
		...
		<div class="flex flex-col flex-1 gap-4 w-full pb-10">
			...
			{:else}
				<div class="flex gap2 items-center">
					...
					<p class="text-center text-neutral-500 p-4">
						Selected User: {matchedUsers.find((user) => user.id === newMember.userid).first_name}
						{matchedUsers.find((user) => user.id === newMember.userid).last_name}
					</p>
				</div>
