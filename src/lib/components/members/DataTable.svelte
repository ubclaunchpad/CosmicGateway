	...
	<div class="overflow-x-auto w-full" slot="content">
		<table class="table dark:bg-neutral-900 border-collapse w-full">
				...
				<tr class="flex border-b flex-1 w-full border-b-neutral-200 dark:border-b-neutral-900">
					{#each columns as column}
						<div class=" min-w-40 flex-1 flex-shrink-0 items-center">
							<th
								class=" px-4 text-md text-black font-bold items-center py-3 h-full dark:text-white"
								>{column.header}</th
							>
						</div>
					{/each}
			...
			<tbody class="border-t-0 flex-1 w-full p-40">
				{#each data as row}
					<div
						class="pb-0.5 flex-1 w-full min-w-40 flex-shrink-0 last:rounded-b overflow-y-hidden first:pt-0.5"
					>
						<tr
						...
						>
							{#each columns as column}
								<td
									class="px-4 py-1 text-sm flex flex-1 text-overflow-ellipsis dark:text-white flex-shrink-0"
								>
